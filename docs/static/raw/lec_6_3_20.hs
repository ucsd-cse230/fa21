...
-- | Eval with Exceptions ------------------------------------------------------ 
-- evalMega :: (MonadError Expr m, MonadState Int m) => Expr -> m Int

-- evalMega :: Expr -> Exn Int 
evalMega :: Expr -> ExnProf Int
evalMega = eval
  where
    eval (Number n)    = return n
    ...
    eval (Def   e n)   = catchError                -- try  
                           (eval e)                   --   { e }
                           (\exn -> return n)         -- (exn) { ...}


-- >>> runExnProf' (evalMega (Def q1 7))
-- "value = 7, count: 2"
-- >>> q1

-- >>> runExnProf' (evalMega (Div (Number 10) (Plus (Number 5) (Number (5)))))
-- "value = 1, count: 2"
--

-- <interactive>:8195:2-20: error:
...
type Profile a = StateT Int Identity a
...
evalProf :: (MonadState Int m) => Expr -> m Int
evalProf = eval 
  where
    eval (Number n)    = return n
    eval (Plus  e1 e2) = do n1 <- eval e1 
...
-- >>> runProfile (evalProf (Div (Number 500) (((Plus (Number 5) (Number (5)))))))
-- "value: 50, count: 2"
--
...
runExnProf' epm = case runStateT (runExceptT epm) 0 of
  Identity (Left e, n)  -> show e ++ " after " ++ show n ++ " operations"
  Identity (Right v, n) -> "value = " ++ show v ++ ", count: " ++ show n
